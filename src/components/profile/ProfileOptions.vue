          ...
          if(error.response.status === 400) {
            ...
            if(data.email) {
              this.$q.notify({
                ...
                position: 'center',
                icon: 'warning',
                message: 'Email invalido ou já cadastrado, por favor use outro email!',
                timeout: 4000
              });
            }
          }
  ...
  <q-card>
    <q-form @submit.prevent="submit">
      ...
      <q-card-section v-show="edit" class="q-gutter-sm">
        <q-input v-model="form.name" label="Nome" :rules="userStore.rules.name" outlined />
        <q-input v-model="form.email" label="Email" :rules="userStore.rules.email" outlined />
      </q-card-section>
